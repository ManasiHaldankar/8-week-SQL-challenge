...
(case when s.order_date >= ms.join_date then 'Y' else 'N' end) as member from sales s left join menu m
on s.product_id = m.product_id
left join members ms on s.customer_id = ms.customer_id
...
else rank() over(partition by customer_id, member order by order_date) end as ranking from cte;
